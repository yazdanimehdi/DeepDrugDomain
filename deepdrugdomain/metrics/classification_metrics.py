...
class AccuracyScoreMetric(BaseMetric):
    def compute(self, prediction: List[int], target: List[int]) -> float:
...
class BrierScoreLossMetric(BaseMetric):
    def compute(self, prediction: List[float], target: List[int]) -> float:
...
class ClassificationReportMetric(BaseMetric):
    def compute(self, prediction: List[int], target: List[int]) -> str:
...
class ConfusionMatrixMetric(BaseMetric):
    def compute(self, prediction: List[int], target: List[int]) -> List[List[int]]:
...
class FbetaScoreMetric(BaseMetric):
    def compute(self, prediction: List[int], target: List[int], beta=1.0) -> float:
...
class MultilabelConfusionMatrixMetric(BaseMetric):
    def compute(self, prediction: List[List[int]], target: List[List[int]]) -> List[List[List[int]]]:
...
class PrecisionRecallFscoreSupportMetric(BaseMetric):
    def compute(self, prediction: List[int], target: List[int]) -> dict:
...
class ROCEnrichmentMetric(BaseMetric):
    def compute(self, prediction: List[float], target: List[int], enrichment_percent: float = 0.005) -> float:
...
class ROCEnrichmentMetric(BaseMetric):
    def compute(self, prediction: List[float], target: List[int], enrichment_percent: float = 0.01) -> float:
        # Determine the number of true positives at the enrichment percentage
        num_total = len(target)
        num_positives = sum(target)
        num_to_select = int(enrichment_percent * num_total)
...
class ROCEnrichmentMetric(BaseMetric):
    def compute(self, prediction: List[float], target: List[int], enrichment_percent: float = 0.02) -> float:
        # Determine the number of true positives at the enrichment percentage
        num_total = len(target)
        num_positives = sum(target)
        num_to_select = int(enrichment_percent * num_total)
...
class ROCEnrichmentMetric(BaseMetric):
    def compute(self, prediction: List[float], target: List[int], enrichment_percent: float = 0.03) -> float:
        # Determine the number of true positives at the enrichment percentage
        num_total = len(target)
        num_positives = sum(target)
        num_to_select = int(enrichment_percent * num_total)
...
class ROCEnrichmentMetric(BaseMetric):
    def compute(self, prediction: List[float], target: List[int], enrichment_percent: float = 0.05) -> float:
        # Determine the number of true positives at the enrichment percentage
        num_total = len(target)
        num_positives = sum(target)
        num_to_select = int(enrichment_percent * num_total)
...
class ROCEnrichmentMetric(BaseMetric):
    def compute(self, prediction: List[float], target: List[int], enrichment_percent: float = 0.10) -> float:
        # Determine the number of true positives at the enrichment percentage
        num_total = len(target)
        num_positives = sum(target)
        num_to_select = int(enrichment_percent * num_total)

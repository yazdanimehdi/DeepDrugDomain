...
class DrugProteinDataset(Dataset):
        ...
        for online, in_memory, preprocess, attribute in all_data:
            ...
            if self._preprocessing_done(preprocess.__class__.__name__, attribute):
                if not in_memory:
                    ...
                    path = preprocess.get_saved_path(self.save_directory, attribute)
                    mapping_data = preprocess.load_preprocessed_to_memory(path)
                    _ = self._load_mapping(preprocess, attribute)
            else:
                info_dict = preprocess.process_and_get_info(data, self.save_directory, in_memory, self.threads, attribute)
    ...
    def _get_data_by_type(self, d_type: str) -> List[Tuple]:
        ...
        if d_type == "drug":
            ...
            raise NotImplementedError(f"'{d_type}' is not implemented. Use either 'drug' or 'protein'.")
    ...
    def _preprocessing_done(self, preprocess_type: str, attribute: str) -> bool:
        ...
        """
        mapping_path = os.path.join(self.save_directory, f"{preprocess_type}_{attribute}_mapping_info.json")
    ...
    def _clean_data(self) -> None:
        ...
        for online, mapping, pre_process, in_mem in combined_attributes:
            none_col_list = pre_process.collect_invalid_data(online, self.data[mapping[0]].unique())
    ...
    def __getitem__(self, index: int) -> Any:
        ...
        data = [
            get_processed_data(online, mapping, pre_process, in_mem, self.data.iloc[index][mapping[0]])
            for online, mapping, pre_process, in_mem in combined_attributes
        ]

        y = torch.tensor(self.data.iloc[index]['Label'])

...
Taken from https://pybiomed.readthedocs.io/en/latest/
"""
...
def GetAAComposition(ProteinSequence):
    ...
    Result = {}
    for i in AALetter:
        Result[i] = round(float(ProteinSequence.count(i)) /
                          LengthSequence * 100, 3)
    return Result
...
def _GetPseudoAAC2(ProteinSequence, lamda=10, weight=0.05):
    ...
    rightpart = []
    for i in range(lamda):
        rightpart.append(_GetSequenceOrderCorrelationFactor(
            ProteinSequence, k=i + 1))
...
def GetAPseudoAAC2(ProteinSequence, lamda=30, weight=0.5):
    ...
    rightpart = []
    for i in range(lamda):
        temp = GetSequenceOrderCorrelationFactorForAPAAC(
            ProteinSequence, k=i + 1)
...
def GetPseudoAAC2(ProteinSequence, lamda=30, weight=0.05, AAP=[]):
    ...
    rightpart = []
    for i in range(lamda):
        rightpart.append(GetSequenceOrderCorrelationFactor(
            ProteinSequence, i + 1, AAP))
    ...
    for i in AALetter:
        for j in AALetter:
            ...
            Result[Dipeptide] = round(
                float(ProteinSequence.count(Dipeptide)) /
                (LengthSequence - 1) * 100, 2
            )
    return Result
...
def GetSequenceOrderCouplingNumberTotal(ProteinSequence, maxlag=30):
    ...
    """
    Tau = {}
    Tau.update(GetSequenceOrderCouplingNumberSW(
        ProteinSequence, maxlag=maxlag))
    Tau.update(GetSequenceOrderCouplingNumberGrant(
        ProteinSequence, maxlag=maxlag))
    return Tau
...
def GetQuasiSequenceOrder2(ProteinSequence, maxlag=30, weight=0.1, distancematrix={}):
    ...
    for index in range(20, 20 + maxlag):
        result["QSO" + str(index + 1)] = round(weight *
                                               rightpart[index - 20] / temp, 6)
...
def GetQuasiSequenceOrder(ProteinSequence, maxlag=30, weight=0.1):
    ...
    """
    result = dict()
    result.update(GetQuasiSequenceOrder1SW(
        ProteinSequence, maxlag, weight, _Distance1))
    result.update(GetQuasiSequenceOrder2SW(
        ProteinSequence, maxlag, weight, _Distance1))
    result.update(
        GetQuasiSequenceOrder1Grant(
            ProteinSequence, maxlag, weight, _Distance2)
    )
    result.update(
        GetQuasiSequenceOrder2Grant(
            ProteinSequence, maxlag, weight, _Distance2)
    )
    return np.array(list(result.values()))
